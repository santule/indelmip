...
def count_indel_events(treefile,indelfastafile):
    ...
    for n in tree.traverse():
        if n.is_leaf() == False:
            ...
            i2_mutations  = count_mutations(parent_node_sequence,current_node_sequence,child_seq_1,child_seq_2)
            total_i2_mut +=  i2_mutations

    return total_i2_mut , round((total_i2_mut / total_ancestors)* 100,2)
...
def main(nwk_file_path,indel_fasta_solution_file):
    ...
    total_i2_mut,percent_ancestor_with_2_mut = count_indel_events(nwk_file_path,indel_fasta_solution_file)
    print(total_i2_mut,percent_ancestor_with_2_mut)
    print(f"Total ancestors with pattern 2 mutations away:{total_i2_mut}")
    print(f"% ancestors with pattern 2 mutations away:{percent_ancestor_with_2_mut}")
    return total_i2_mut,percent_ancestor_with_2_mut

if __name__ == "__main__":
  total_i2_mut,percent_ancestor_with_2_mut = main(nwk_file_path,indel_fasta_solution_file)
