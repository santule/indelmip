...
def mip_processing(extant_data,ancestor_data,neighbor_dict,optimal_constraint,output_folder_location):
    ...
    if optimal_constraint == 'Y':
        with open(output_folder_location + 'optimal_edges.pkl','rb') as f:
    ...
    if is_sat:
        ...
        with open(output_folder_location + 'optimal_edges.pkl','wb') as f:
...
def main(extant_sequence_file,nwk_file_path,output_folder_location,optimal_constraint):
    ...
    # 2 - build model
    mip_processing(extant_data,ancestor_data,neighbor_dict,optimal_constraint,output_folder_location)
