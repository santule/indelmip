...
class PhyloTreeMIP:
    ...
    def remove_optimal_constraint(self,optimal_edge_list):
        optimal_edge_sol = []
        for opt_edge in  optimal_edge_list:
            optimal_edge_sol.append(self.edges[opt_edge])
        self.m.addConstr(quicksum(optimal_edge_sol) <= len(optimal_edge_list) - 1)
    ...
    def get_solution(self):
        ...
        # get the path for all ancestors
        for ancestor in self.ancestor_data[0]:
            ...
            # get the edges that are 1 in the solution
            # START NODES
            for fwd_back_pos in ancestor_node_type.get(('ANCESTOR','start')):
                for pos_to in ancestor_fwd_edges[fwd_back_pos]:
                    edge_id = (ancestor,fwd_back_pos,pos_to)
                    if self.edges[edge_id].X == 1:
                        optimal_edges_list.append(edge_id)
        ...
        return all_node_paths,optimal_edges_list
